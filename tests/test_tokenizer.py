...
def mock_tokenizer():
    ...
    """
    with patch(
        "vad_code.infrastructure.tokenizer.AutoTokenizer.from_pretrained"
        ...
        mock_tok_instance.encode.side_effect = lambda text, add_special_tokens=False: [
            f"token_{i}" for i in range(len(text))
        ]
