...
def test_run_tests_failure(tools, tmp_path):
    ...
    with patch("subprocess.run") as mock_run:
        mock_run.return_value = MagicMock(
            returncode=1, stdout="1 failed in 0.5s", stderr="AssertionError: 1 != 2"
        )
        result = tools.run_tests(path="tests/")
...
def test_format_code_success(tools, tmp_path):
    ...
    with patch("subprocess.run") as mock_run:
        mock_run.return_value = MagicMock(
            returncode=0, stdout="1 file reformatted", stderr=""
        )
...
def test_install_package_success(tools, tmp_path):
    ...
    with patch("subprocess.run") as mock_run:
        mock_run.return_value = MagicMock(
            returncode=0, stdout="Successfully installed requests-2.31.0", stderr=""
        )
...
def test_install_package_upgrade(tools, tmp_path):
    ...
    with patch("subprocess.run") as mock_run:
        mock_run.return_value = MagicMock(
            returncode=0, stdout="Successfully installed requests-2.32.0", stderr=""
        )
