...
class FindFilesSchema(BaseModel):
    ...
    pattern: str = Field(
        ..., description="Шаблон имени файла, например '*.py' или 'test_*.py'"
    )
...
class TailFileSchema(BaseModel):
    ...
    path: str = Field(..., description="Путь к файлу")
    num_lines: int = Field(
        20, description="Количество строк с конца файла", ge=1, le=500
    )
...
class HeadFileSchema(BaseModel):
    ...
    path: str = Field(..., description="Путь к файлу")
    num_lines: int = Field(
        20, description="Количество строк с начала файла", ge=1, le=500
    )
...
class GetProjectStatsSchema(BaseModel):
    ...
    file_glob: str = Field(
        "*.py", description="Маска файлов для анализа, например *.py"
    )
...
class FormatCodeSchema(BaseModel):
    ...
    path: str = Field(".", description="Путь к файлу или директории")
    tool: str = Field(
        "black", description="Инструмент форматирования: black, autopep8, isort"
    )
...
class InstallPackageSchema(BaseModel):
    ...
    package: str = Field(
        ...,
        description="Имя пакета для установки (например, 'requests' или 'numpy>=1.21')",
    )
    upgrade: bool = Field(False, description="Обновить пакет до последней версии")
    user_install: bool = Field(
        False, description="Установить в пользовательскую директорию"
    )
...
class UninstallPackageSchema(BaseModel):
    ...
    package: str = Field(
        ..., description="Имя пакета для удаления (например, 'requests')"
    )
...
class ListPackagesSchema(BaseModel):
    ...
    show_upgradable: bool = Field(
        False, description="Показать только пакеты, доступные для обновления"
    )
...
class UpdatePackageSchema(BaseModel):
    ...
    user_install: bool = Field(
        False, description="Обновить в пользовательской директории"
    )
